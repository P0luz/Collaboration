...
| 工具 | 对应 API | 语义 |
|------|----------|------|
| `collaboration.capabilities`   | GET /capabilities/{room} | Read-only room plan, relay, and feature capability metadata |
...
| `collaboration.audit_export`   | GET /audit/{room}/export?fmt=jsonl | 导出 JSONL 审计日志 |
...
| `collaboration.rehearsal_evidence` | GET /rehearsal/{room}/evidence?limit=N | 导出 M6 演练证据 bundle |
...
- `rehearsal_evidence` 当前为 M6 演练辅助投影;只整理已有元数据,不自动判定真实任务是否完成。

## Room Policy 字段

`room/create` 支持商业化预留字段,不涉及计费:
...
```json
{
  ...
  "max_participants": 10,
  "relay_mode": "local | self_hosted | saas | private",
  "audit_retention_days": 30,
  "policy_rules_enabled": true
}
```

默认 `free` 计划限制 2 人;`team` / `pro` 默认 10 人;`enterprise` 默认 50 人。显式传入
`max_participants` 时会覆盖计划默认值,便于本地演练或自建部署验证。
...
```powershell
Invoke-RestMethod http://localhost:8080/api/collaboration/audit/<room_id>/export?fmt=jsonl
```
...
`collaboration.capabilities` maps to `GET /capabilities/{room}`. It is read-only
metadata for clients and agents: current room policy, relay mode/connection
state, and M7 feature flags such as audit export, dashboard, rehearsal evidence,
brand boundary checking, and policy rules.
