...
验证 AI agent 在 Pair Mesh 下是否遵守协作协议的场景清单。
...
6. **hook 拦截响应**:commit 被 hook 拦时,AI 应解析 `PAIR_MESH_ACTION` 并执行 `then` 指令。

## 对应自动化测试

上述协议层逻辑由以下单测覆盖(逻辑层,非真实 AI 行为):
- `tests/pair_mesh/test_locks.py` —— 声明/冲突/完成/提升/过期
- `tests/pair_mesh/test_queues.py` —— 排队顺序/提升
- `tests/pair_mesh/test_extend_lock.py` —— 扩展/部分冲突
- `tests/pair_mesh/test_git_gate.py` —— hook 拦截决策
- `tests/pair_mesh/test_router.py` —— 端到端 HTTP 流程
