...
from backend.collabration import events, locks, queues, rooms
from backend.collabration.app import app
...
def test_full_flow(client):
    # 建房 + 两人加入
    assert client.post("/api/collabration/room/create", json={"room_id": "R"}).json()["status"] == "created"
    client.post("/api/collabration/room/join", json={"room_id": "R", "name": "Alice", "agent": "Claude Code"})
    client.post("/api/collabration/room/join", json={"room_id": "R", "name": "Bob", "agent": "Codex"})

    # Alice 声明 -> clear
    d1 = client.post("/api/collabration/intent/declare", json={
    ...
    # Bob 声明同文件 -> conflict
    d2 = client.post("/api/collabration/intent/declare", json={
    ...
    st = client.get("/api/collabration/status/R").json()
    ...
    w = client.post("/api/collabration/intent/wait_for_clear", json={
    ...
    # Alice 完成 -> Bob 上位
    done = client.post("/api/collabration/intent/done", json={
        "lock_id": d1["lock_id"], "summary": "done",
    }).json()
    assert done["promoted"][0]["owner"] == "Bob"

    # hook/check:Bob 现在合法持有
    hc = client.post("/api/collabration/hook/check", json={
        "room_id": "R", "requester": "Bob", "staged_files": ["src/main.py"],
    }).json()
    assert hc["blocked"] is False

    # hook/check:Alice 已无锁 -> 阻止
    hc2 = client.post("/api/collabration/hook/check", json={
        "room_id": "R", "requester": "Alice", "staged_files": ["src/main.py"],
    }).json()
    assert hc2["blocked"] is True
    assert "COLLABRATION_ACTION" in hc2

    # 事件流非空
    ev = client.get("/api/collabration/events/R").json()
    assert len(ev["events"]) > 0


def test_status_404(client):
    assert client.get("/api/collabration/status/ghost").status_code == 404


def test_extend_via_api(client):
    client.post("/api/collabration/room/create", json={"room_id": "R"})
    d = client.post("/api/collabration/intent/declare", json={
        "room_id": "R", "owner": "Alice", "files": ["a.py"], "intent": "x",
    }).json()
    ext = client.post("/api/collabration/intent/extend", json={
