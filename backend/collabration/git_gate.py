...
不做什么:不发起 HTTP、不读 git;那些由 scripts/collabration-hooks/* 脚本负责。
...
def build_hook_feedback(results: list[dict]) -> HookFeedback:
    ...
    lines += [
        "",
        "请先调用 collabration.wait_for_clear,等待释放后 git pull --rebase 再重新 declare_intent。",
    ]
