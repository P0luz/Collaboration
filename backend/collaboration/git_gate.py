...
不做什么:不发起 HTTP、不读 git;那些由 scripts/collaboration-hooks/* 脚本负责。
...
def build_hook_feedback(results: list[dict]) -> HookFeedback:
    ...
    lines = ["[Collaboration] commit 被阻止。", ""]
    holders: list[dict] = []
    for b in blocked:
        status = b.get("status")
        if status == "no_lock":
    ...
    lines += [
        "",
        "请先调用 collaboration.wait_for_clear,等待释放后 git pull --rebase 再重新 declare_intent。",
    ]
