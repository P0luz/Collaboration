...
> 给自己的工作笔记 + 计划表。本轮目标:在 `p:/Savoir-pairMesh` 从零搭一个**独立**的 Pair Mesh backend,
> M1(协作底座)+ M2(本地最小协议)全部完成。每做一步跑一次测试,工作留痕。

## 关键决定(已与合作者对齐 2026-06-30)

1. **零代码起步,独立项目**:不 clone Open WebUI / Savoir-Fair。Pair Mesh 作为独立 FastAPI 服务实现,
   不依赖 Open WebUI 任何代码。
...
│       ├── queues.py          # 文件排队
│       ├── events.py          # 事件记录
│       ├── router.py          # FastAPI router(对应 spec 的 pair_mesh.py)
│       ├── watcher.py         # M4 占桩
│       ├── git_gate.py        # M4 占桩
│       ├── relay.py           # M3 占桩
│       ├── policy.py          # 预留占桩
...
- [x] 0. 工程脚手架:requirements.txt、pytest 配置、backend/pair_mesh/__init__.py、tests 包
...
- [x] 9. 文档:CLAUDE.md / AGENTS.md / THIRD_PARTY_NOTICES.md / docs/pair-mesh/*(5 个)
- [x] 10. 全量验收:pytest 42 passed + 端点冒烟 200 + 13 个文件头全有功能说明

## 验收结论(2026-06-30)

- 全量:`pytest tests/` → **42 passed, 1 warning**(starlette 弃用警告,无害)。
- 端点冒烟:TestClient 打 / 与 /api/pair-mesh/* → 200。
- 文件头:backend/pair_mesh/ 全部 13 个 .py 均含"是什么/做什么/不做什么/对外暴露"。
- 与 spec 的有意偏离(已对齐):
