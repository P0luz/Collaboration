...
> 给自己的工作笔记 + 计划表。本轮目标:在 `p:/Savoir-pairMesh` 从零搭一个**独立**的 Collaboration backend,
> M1(协作底座)+ M2(本地最小协议)全部完成。每做一步跑一次测试,工作留痕。

## 关键决定(已与合作者对齐 2026-06-30)

1. **零代码起步,独立项目**:不 clone Open WebUI / Savoir-Fair。Collaboration 作为独立 FastAPI 服务实现,
   不依赖 Open WebUI 任何代码。
...
│       ├── queues.py          # 文件排队
│       ├── events.py          # 事件记录
│       ├── router.py          # FastAPI router(对应 spec 的 collaboration.py)
│       ├── watcher.py         # M4 占桩
│       ├── git_gate.py        # M4 占桩
│       ├── relay.py           # M3 占桩
│       ├── policy.py          # 预留占桩
...
- [x] 0. 工程脚手架:requirements.txt、pytest 配置、backend/collaboration/__init__.py、tests 包
...
- [x] 9. 文档:CLAUDE.md / AGENTS.md / THIRD_PARTY_NOTICES.md / docs/collaboration/*(5 个)
- [x] 10. 全量验收:pytest 42 passed + 端点冒烟 200 + 13 个文件头全有功能说明

## 验收结论(2026-06-30)

- 全量:`pytest tests/` → **42 passed, 1 warning**(starlette 弃用警告,无害)。
- 端点冒烟:TestClient 打 / 与 /api/collaboration/* → 200。
- 文件头:backend/collaboration/ 全部 13 个 .py 均含"是什么/做什么/不做什么/对外暴露"。
- 与 spec 的有意偏离(已对齐):
...
- 按合作者要求,全项目命名 `pair_mesh`/`pair-mesh`/`Pair Mesh`/`PAIR_MESH` 统一改成 **collaboration**:
  - 包/模块:`backend/pair_mesh/` → `backend/collaboration/`,import 全部跟改。
  - 测试目录:`tests/pair_mesh/` → `tests/collaboration/`。
  - 文档目录:`docs/pair-mesh/` → `docs/collaboration/`(secret 子目录一并搬移)。
  - hook 脚本:`scripts/pair-mesh-hooks/` → `scripts/collaboration-hooks/`。
  - 环境变量:`PAIR_MESH_*` → `COLLABORATION_*`;API 前缀 `/api/pair-mesh/` → `/api/collaboration/`。
  - MCP 工具名:`pair_mesh.xxx` → `collaboration.xxx`;hook 字段 `PAIR_MESH_ACTION` → `COLLABORATION_ACTION`。
- 改名后 pytest 仍 42 passed,端点冒烟 200。
- 注:`docs/collaboration/secret/` 里的原始资料(spec、工程规范)文件名保留原样,不入库。
